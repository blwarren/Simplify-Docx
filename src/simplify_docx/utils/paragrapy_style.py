...
def get_num_style(p, doc):
    ...
    if getattr(p, "pPr", None) is not None and p.pPr.numPr is not None and p.pPr.numPr.numId is not None:
        ...
        return abstractNumbering.find(f"w:lvl[@w:ilvl='{p.pPr.numPr.ilvl.val}']", np.element.nsmap)
    return None
...
def get_paragraph_ind(p, doc):
    ...
    * Direct Formatting
    ...
    if pStyle is not None and pStyle.pPr is not None and pStyle.pPr.ind is not None:
